...
def get_serial_path():
	for i in range(10):
		fn = '/dev/ttyUSB%i' % i # Default NodeMCU ESP-12E serial interface.
		if os.path.exists(fn):#, 'Device not found at %s. Are you sure it is connected?' % fn
			return fn
...
def get_login():
    ...
    cmd = '%s\n' % (COMMAND_GET_LOGIN,)
    ser.write(cmd.encode())
    stream_serial_input(ser, lines=1)
